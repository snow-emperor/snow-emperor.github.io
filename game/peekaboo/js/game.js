...
class PeekabooGame {
    ...
    init() {
        ...
        window.addEventListener('online', () => {
            console.log('网络连接恢复');
            this.showMessage('网络连接已恢复，正在检查并恢复连接...', 'info');
            // 网络恢复后检查并恢复连接
            setTimeout(() => {
        ...
        window.addEventListener('offline', () => {
            console.log('网络连接丢失');
            this.showMessage('网络连接已丢失，请检查网络设置', 'error');
        });
            ...
            this.peer = new Peer(this.localPlayer.id, {
                ...
                config: {
                    iceServers: [
                        ...
                        { 
                            urls: 'turn:turn.cloudflare.com:50000?transport=tcp',
                            username: 'user',
                            credential: 'pass'
                        }
                    ],
                    iceCandidatePoolSize: 20, // 增加候选池大小以提高连接成功率
                    iceTransportPolicy: 'all', // 允许所有类型的ICE候选，包括IPv4和IPv6
                    iceConnectionReceivingTimeout: 45000, // 增加接收超时时间到45秒
                    iceConnectionTimeout: 30000 // 增加连接超时时间到30秒
                }
            });
    ...
    connectToPlayers() {
        ...
        for (const player of this.gameData.players) {
            if (player.id !== this.localPlayer.id) {
                this.attemptConnection(player, 7); // 最多重试7次，适应复杂网络环境
            }
        }
    }
    ...
    attemptConnection(player, retriesLeft) {
        try {
            ...
            const connectionTimeout = setTimeout(() => {
                if (!conn.open) {
                    console.warn(`连接到 ${player.id} 超时`);
                    conn.close();
                    
                    // 如果还有重试次数，继续重试
                    if (retriesLeft > 1) {
                        console.log(`连接到 ${player.id} 超时，${retriesLeft-1} 次重试机会剩余`);
                        setTimeout(() => {
                            this.attemptConnection(player, retriesLeft - 1);
                        }, 5000); // 蜂窝网络需要更长的重试间隔
                    } else {
                        this.showMessage(`与玩家 ${player.name} 连接超时，请检查网络连接后重试`, 'error');
                    }
                }
            }, 45000); // 45秒超时，适合蜂窝网络
    ...
    checkAndRestoreConnections() {
        ...
        for (const [playerId, conn] of this.connections) {
            if (!conn.open) {
                ...
                // 查找玩家信息
                const player = this.gameData.players.find(p => p.id === playerId) || { name: playerId };
                this.attemptConnection(player, 5); // 尝试重新连接
            }
        }
