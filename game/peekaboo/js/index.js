...
class PeekabooGameIndex {
    ...
    async init() {
        ...
        if (navigator.connection) {
            navigator.connection.addEventListener('change', () => {
                ...
                console.log('网络类型已变更为:', networkType);
                
                if (networkType === 'slow-2g' || networkType === '2g') {
                    this.showMessage('检测到慢速网络连接，游戏体验可能受影响', 'warning');
                } else if (networkType === '3g') {
                    this.showMessage('使用3G网络，连接可能不稳定', 'info');
                } else if (networkType === '4g') {
                    this.showMessage('使用4G网络，连接质量良好', 'success');
                }
            });
        }
    ...
    connectToPeer() {
        ...
        if (navigator.connection) {
            ...
            // 如果是慢速网络，提供提示
            if (networkType === 'slow-2g' || networkType === '2g') {
                this.showMessage('检测到慢速网络连接，连接可能需要更长时间', 'warning');
            } else if (networkType === '3g') {
                this.showMessage('使用3G网络连接，可能需要稍长时间', 'info');
            }
        }
    ...
    /**
     * 连接到其他玩家
     * @param {string} peerId - 目标玩家的Peer ID
     */
    async connectToPlayer(peerId) {
        if (!this.peer) {
            this.showMessage('请先初始化网络', 'error');
            return;
        }
        
        // 检查是否已连接
        if (this.connections.has(peerId)) {
            this.showMessage('已连接到此玩家', 'warning');
            return;
        }
        
        try {
            this.showMessage('正在尝试连接...', 'info');
            ...
            conn.on('close', () => {
                this.connections.delete(peerId);
                this.showMessage(`与玩家 ${peerId} 的连接已断开`, 'warning');
            });
            
            conn.on('error', (err) => {
                console.error('连接错误:', err);
                this.showMessage(`连接错误: ${err.message}`, 'error');
            });
            ...
            this.showMessage(`成功连接到玩家 ${peerId}`, 'success');
        } catch (error) {
    ...
    async smartReconnect(peerId) {
        ...
        if (networkType === 'cellular' || networkType === 'slow-2g' || networkType === '2g') {
            maxRetries = 5;
            baseDelay = 2000;
        } else if (networkType === '3g') {
            maxRetries = 4;
        ...
        for (let i = 0; i < maxRetries; i++) {
            try {
                console.log(`尝试连接到 ${peerId}，第 ${i+1}/${maxRetries} 次`);
        ...
        throw new Error(`连接失败，已尝试 ${maxRetries} 次。最后错误: ${lastError.message}`);
    }
    ...
    async attemptReconnection() {
        ...
        console.log('尝试重新连接所有断开的连接');
        
        // 获取所有应该连接的玩家ID
        const allPlayerIds = [this.localPlayer.id, ...this.players.map(p => p.id)];
        
        for (const playerId of allPlayerIds) {
            if (playerId !== this.localPlayer.id && !this.connections.has(playerId)) {
                try {
                    const player = this.players.find(p => p.id === playerId) || { name: playerId };
                    console.log(`尝试重新连接到玩家 ${player.name || playerId}`);
                    await this.connectToPlayer(playerId);
                } catch (error) {
                    console.error(`重新连接到玩家 ${playerId} 失败:`, error);
                }
            }
        }
    }
    ...
    async checkConnectionHealth() {
        ...
        for (const [playerId, conn] of this.connections) {
            if (!conn.open) {
                console.log(`检测到与玩家 ${playerId} 的连接已断开，尝试重新连接`);
    ...
    attemptConnection(player, playerId, retriesLeft) {
        try {
            // 设置更长的连接超时时间，特别针对蜂窝网络
            const conn = this.peer.connect(playerId, { 
                reliable: true,
                // 增加连接超时时间，蜂窝网络需要更长时间建立连接
                timeout: 30000,
                // 使用更宽松的配置选项
                serialization: 'json',
                // 启用更积极的重连策略
