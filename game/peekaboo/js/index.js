...
class PeekabooGameIndex {
    ...
    handleMessage(peerId, data) {
        switch (data.type) {
            ...
            case 'GAME_SETTINGS_UPDATE':
                // 兼容旧格式和新格式
                if (data.payload.settingName && data.payload.value !== undefined) {
                    // 旧格式
                    this.gameSettings[data.payload.settingName] = data.payload.value;
    ...
    connectToPeer() {
        ...
        this.attemptConnection(player, otherId, 3); // 最多重试3次
    }
    ...
    attemptConnection(player, playerId, retriesLeft) {
        try {
            const conn = this.peer.connect(playerId, { reliable: true });
            ...
            conn.on('error', (error) => {
                ...
                if (retriesLeft > 1) {
                    ...
                    this.handleDisconnection(playerId);
                    // 提示用户连接最终失败
                    this.showMessage(`与玩家 ${player.name || playerId} 连接失败: ${error.message}`, 'error');
                }
            });
