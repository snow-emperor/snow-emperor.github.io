...
export class Player {
  ...
  break() {
    const now = Date.now();
    if (now - this.lastBreakTime < this.calculateMineTime()) return;
    
    const hit = this.raycast(4);
    if (!hit) return;
  ...
  calculateMineTime() {
    const hit = this.raycast(4);
    if (!hit) return mineTime(this.mode, this.selectedID, this.difficulty);
    
    // 检查是否是物体并计算挖掘时间
    if (this.chunkMgr.isObject(hit.x, hit.y, hit.z)) {
      const objectType = this.chunkMgr.getObjectType(hit.x, hit.y, hit.z);
      // 物体的挖掘时间基于其硬度
      const hardness = getObjectHardness(Object.keys(objectType)[objectType]);
    ...
    return mineTime(this.mode, this.selectedID, this.difficulty);
  }
